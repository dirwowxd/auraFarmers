...
public class VentanaConsultaTerminales extends JFrame {
    ...
    private void construirInterfaz() {
        ...
        modeloTabla = new DefaultTableModel(new String[]{"Nombre", "Calle", "Número", "Comuna"}, 0) {
            @Override
    ...
    private void cargarTerminales() {
        try {
            String[][] datos = controladorEmpresas.listTerminales();
            modeloTabla.setRowCount(0);
            for (String[] fila : datos) {
                modeloTabla.addRow(fila);
            }
            lblCantidad.setText("Terminales encontrados: " + datos.length);
